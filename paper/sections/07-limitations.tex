...
Twelve songs, balanced by class. The separation is complete---6/6 against
0/6---but the sample is small enough that the effect size, not merely its
direction, needs confirmation at scale.

\paragraph{Single seed.}
Results are from one training run per configuration. Variance across seeds is
unmeasured.
